...
| CX-01 | 待实施 | 建立隔离的 Codex SDK POC，不接业务 API | POC 脚本/报告、固定依赖、最小 CODEX_HOME | Gate A 全通过 |
| CX-02 | 待实施 | 构建 Codex Agent sidecar 与通用 AgentClient 契约 | Sidecar、健康检查、流式运行/停止接口 | 单次 Thread/Turn/interrupt 可用 |
